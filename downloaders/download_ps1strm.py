"""Script to download all of PS1-STRM's catalog files in parallel, with resume support."""
import os
import time
import argparse
import requests
...
load_dotenv()
# Retrieve output directory from environment variable or use ./ps1_strm/
PS1_STRM_OUTPUT_DIR = f"{os.getenv('OUTPUT_DIR','.')}/ps1_strm/"

BASE_URL = "https://archive.stsci.edu/hlsps/ps1-strm/"
EXTRA_FILES = [
    "hlsp_ps1-strm_ps1_gpc1_all_multi_v1_readme.txt",
    "hlsp_ps1-strm_ps1_gpc1_all_multi_v1_md5sum.txt",
]

parser = argparse.ArgumentParser(description="Download all of PS1-STRM's catalog files in parallel.")
parser.add_argument("--output-dir", type=str, default=PS1_STRM_OUTPUT_DIR, help="Directory to save downloaded files")
...
def get_urls(main_url=BASE_URL):
    response = requests.get(main_url, timeout=(10, 120))
    ...
    for line in lines:
        if 'href="' in line:
            ...
            if filename.endswith('_cat.csv.gz'):
...
if __name__ == "__main__":
    ...
    nb_processes = max(1, min(args.processes, cpu_count() - 2))

    os.makedirs(output_dir, exist_ok=True)

    failures = []
    for extra in EXTRA_FILES:
        failed = download_file((BASE_URL + extra, output_dir, args.retries))
        if failed:
